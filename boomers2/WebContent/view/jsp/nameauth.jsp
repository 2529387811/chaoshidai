...
function Delete(realid){
    var real={"realid":realid};
	$.ajax({
		type:'post',
		url:'${pageContext.request.contextPath }/deleteReal',
...
<div id="w1">
        <div class="w">
        	<div class="col-lg-2" id="p1">
        		...
        		<a href="${pageContext.request.contextPath }/nameauth" class="addr"><div class="p3 w16" onclick="q('s16',this)" style="color: red; background-color: white;"><b style="background-position: 0 -325px;"></b><span>管理实名认证</span></div></a>
         	...
         	<!--绑定手机-->
        <div class="col-lg-10" class="q"  id="d16">
        		<div id="p2">
        			<span>实名认证</span>
        		...
        		<div id="p7" style="">
        			<div style="height: 1200px;">
        				<p id="p16" class="fr"  style="margin-right: 140px; margin-top: 40px;">新增</p>
        				<br />
        				
        				<table id="t5" style="text-align: center; margin-top: 40px;">
        					<tr style="border: 1px solid #dbd6d0;">
        						<td>姓名</td>
        						<td>身份证号码</td>
        						<td>验证状态</td>
        						<td>操作</td>
        					</tr>
        					<c:if test="${tbRealName!=null }">
							<c:forEach var="tbRealName" items="${tbRealName }">
        					<tr style="border: 1px solid #dbd6d0;">
        						<td>*${tbRealName.name.substring(1,tbRealName.name.length()) }</td>
        						<td>${tbRealName.idcardno.substring(0,6)  }***********${tbRealName.idcardno.substring(17,tbRealName.idcardno.length())  }</td>
        						<td>${tbRealName.verificationstatus }</td>
        						<td><a class="delBtn">删除</a></td>
        					</tr>
        					<section class="my_model">
								<p class="title">删除<span class="closeModel">X</span></p>
								<p>确认删除吗？删除后可能会影响该收货人的订单正常发货</p>
								<div class="opBtn">
									<a onclick="Delete(${tbRealName.realid})" class="dialog-sure">确定</a>
        				...
        				<!-- 新增 -->
						<!-- 真实姓名：<input type="text" id="name" name="name" /><br/>
						身份证号码：<input type="text" id="idcardno" name="idcardno" /><br/>
						<div class="text-box1">
						            验证码：<input type="text">
						    <div style="display:none;" id="hint2">2</div>
						</div>
						<div class="authcode">
						    <a id="getPhoneCode"><input type="button" value="获取验证码" /></a>
						</div>
						<input type="button" value="取消" /><input type="button" id="btn1" value="保存" />
						 -->
...
$(function(){
	...
	$("#btn1").click(function(){
		...
		if($value == ""){
			...
			var insertReal={"name":$name,"idcardno":$idcardno};
			$.ajax({
				type:'post',
				url:'${pageContext.request.contextPath }/insertReal',
				contentType:'application/json;charset=utf-8',
				dataType:'json',
				processData:false,
				headers:{
				...
				data:JSON.stringify(insertReal),
				success:function(data){//返回json结果
					if(data.name!=null && data.name!=""){
						alert("添加成功！！！");
						window.location.href="${pageContext.request.contextPath }/nameauth";
					}else{
						alert("添加失败,身份证有误！！！");
					}
				},error:function(data){
