...
$(function(){
	...
	$("#btn1").click(function(){
		...
		if($value == ""){
			...
			$htin.css({"display":"none"});
			$(".text-box1").css({"border":"1px solid #e0e0e0","box-shadow":"none"});
			window.location.href="${pageContext.request.contextPath }/mobile_bind2";
		}
		else if(a!=$value){
			$htin.text("验证码不正确");
			$htin.css({"display":"block","color":"red"});
