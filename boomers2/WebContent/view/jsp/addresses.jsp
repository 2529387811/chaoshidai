...
function Delete(addressid){
    var site={"addressid":addressid};
	$.ajax({
		type:'post',
		url:'${pageContext.request.contextPath }/deleteSite',
		...
		success:function(){//返回json结果
			window.location.href="${pageContext.request.contextPath }/selectSite";
		},error:function(data){
			console.log(data);
		}
	});
...
function Insert(){
    var site={"consignee":$("#consignee").val(),"address":$("#province").val()+"-"+$("#city").val()+"-"+$("#area").val(),"detailedads":$("#dizhi").val(),"conphone":$("#phone").val()};
    $.ajax({
		type:'post',
		url:'${pageContext.request.contextPath }/insertSite',
		contentType:'application/json;charset=utf-8',
		dataType:'json',
		processData:false,
		headers:{
		...
		success:function(data){//返回json结果
			if(data.consignee==""){
				alert("收货人姓名不能为空！！！");
			}else if(data.detailedads==""){
				alert("地址不能为空！！！");
			}else if(data.conphone==""){
				alert("手机号不能为空！！！");
			}else if(data.conphone.length!=11){
				alert("手机号长度错误！！！");
			}else{
				alert("添加成功！！！");
				window.location.href="${pageContext.request.contextPath }/selectSite";
...
function Update(addressid){
    var site={"addressid":addressid,"consignee":$("#consignee").val(),"address":$("#province").val()+"-"+$("#city").val()+"-"+$("#area").val(),"detailedads":$("#dizhi").val(),"conphone":$("#phone").val()};
    $.ajax({
		type:'post',
		url:'${pageContext.request.contextPath }/submitUpdateSite',
		contentType:'application/json;charset=utf-8',
		dataType:'json',
		processData:false,
		headers:{
		...
		success:function(data){//返回json结果
			if(data.consignee==""){
				...
				alert("修改成功！！！");
				window.location.href="${pageContext.request.contextPath }/updateSite?addressid="+data.addressid;
			}
		},error:function(data){
			console.log(data);
		}
...
<div id="w1">
        <div class="w">
        	<div class="col-lg-2" id="p1">
        		...
        		<a href="${pageContext.request.contextPath }/gift_card" class="gift"><div class="p3 w12" onclick="q('s12',this)"><b style="background-position: 0 -450px;"></b><span>礼品卡兑换</span></div></a>
        		...
        		<a href="${pageContext.request.contextPath }/selectSite" class="addr"><div class="p3 w17" onclick="q('s17',this)" style="color: red; background-color: white;"><b style="background-position: 0 -325px;"></b><span>管理收货地址</span></div></a>
        	...
        	<div class="col-lg-10" class="q" id="d17">
        		<div id="p2">
        			<span>管理收货地址</span>
        		...
        		<div id="p7" style="">
        			<div style="height: 1200px;">
        				<table id="t5">
        					<tr>
        						<td class="fr" style="font-size: 18px; font-weight: 700px; margin-left: -15px;">新增收货地址</td>
        						<td id="t6">&nbsp;&nbsp;&nbsp;&nbsp;</td>
        					</tr>
        					<tr>
        						<td class="fr">*收件人姓名：</td>
        						<td id="t6">&nbsp;&nbsp;&nbsp;&nbsp;
        							<span><input type="text" id="consignee" style="border: 1px solid #ccc; border-radius: 5px; position: absolute;" /></span>
        						</td>
        					</tr>
        					<tr>
        						<td class="fr">*收货地址：</td>
        					...
        					<tr>
        						<td class="fr">*详细地址：</td>
        						<td id="t6">&nbsp;&nbsp;&nbsp;&nbsp;<textarea  id="dizhi" style=" margin-top:16px; position: absolute; width: 50%; height: 100px; line-height: 20px;"></textarea><br /><br /><br />
        					...
        					<tr>
        						<td class="fr">*手机号码：</td>
        						<td id="t6">&nbsp;&nbsp;&nbsp;&nbsp;
        							<span><input type="text" id="phone" style="border: 1px solid #ccc; border-radius: 5px; position: absolute;" /></span>
        							<br />
        							<input type="button" onclick="Insert()" value="保存收货地址" />
        						</td>
        					</tr>
        					<tr>
        						<td id="t6">&nbsp;&nbsp;&nbsp;&nbsp;</td>
	...
	<c:if test="${sites!=null }">
		<c:forEach var="sites" items="${sites }">
			<tr class="order_lists">
				<td>*${sites.consignee.substring(1,sites.consignee.length()) }</td>
				<%-- <td>${sites.address.split("-") }</td> --%>
				<td>
					<c:forEach items="${sites.address.split('-') }" var="address">
						<c:if test="${!address.equals('null')}">
							${address}
						</c:if>
					</c:forEach>
					${sites.detailedads }
				</td>
				<td>${sites.conphone.substring(0,3) }****${sites.conphone.substring(7,sites.conphone.length()) }</td>
				<td><a href="${pageContext.request.contextPath }/updateSite?addressid=${sites.addressid}">修改</a>&nbsp;<a class="delBtn">删除</a></td>
			</tr>
			<section class="my_model">
				<p>确认删除该地址吗？</p>
				<div class="opBtn">
					<a onclick="Delete(${sites.addressid})" class="dialog-sure">确定</a>
					<a class="dialog-close">关闭</a>
				</div>
			</section>
		</c:forEach>
	</c:if>
